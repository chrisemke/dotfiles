(define-module (dev python-dev)
  #:use-module ((guix profiles)										#:select (packages->manifest))
  #:use-module ((gnu packages base)								#:select (coreutils grep which))
  ...
  #:use-module ((gnu packages package-management) #:select (pipx))
  #:use-module ((gnu packages pkg-config)					#:select (pkg-config))
  #:use-module ((gnu packages python-xyz)					#:select (python-rassumfrassum))
...
(packages->manifest
 (list
  ...
  ;; Python software to install libs globally (global virtualenv)
  pipx
